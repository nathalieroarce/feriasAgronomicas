...
export class ProductsManagementService {
   ...
   * 
   * @param image 
   * @param data all product data
   * @param imageChange true if the image change, false in other case
   */
  public updateProduct(image: File, data: Array<any>,imageChange: any, previousImageImageUrl: any){

    const formData: FormData = new FormData();
    formData.append('imageChange',imageChange);
    formData.append('oldImageUrl',previousImageImageUrl);
    formData.append('productID', data[0]);
    formData.append('name', data[1]);
